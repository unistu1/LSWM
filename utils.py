...
class SaliconT(Dataset):
    def __init__(self,root,df_x, df_y,transform=None,size=(352,352)) -> None:
        ...
        # self.img_transform1 = transform if transform is not None else transforms.Compose([
        #     transforms.Resize(size),
        #     transforms.ToTensor(),
        # ])
        self.size = size 
    ...
    def __getitem__(self, idx):
        ...
        # map4 = cv2.resize(map,(self.size[0]//2,self.size[1]//2))
        map = cv2.resize(map,(self.size[1],self.size[0]))
        # image1 = image.astype('float')
        # transform
        image2 = self.img_transform(image)
        # image1= self.img_transform1(image)
        #如果像素中的数字大于1 就除以255 进行归一化。
        if np.max(map) > 1.0:
            map = map / 255.0
        assert np.min(map) >= 0.0 and np.max(map) <= 1.0

        return image2,torch.FloatTensor(map),torch.FloatTensor(map2),torch.FloatTensor(map3)
